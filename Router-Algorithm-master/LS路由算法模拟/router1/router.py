...
class RouteTable:
    ...
    def init_line_state(self, routers_list):
        line_state = {}
        for router in routers_list:
            line_state[tuple(router)] = init_each_row(routers_list)  # routers_list是一个列表
    ...
    def add_to_line_state(self, addr, online_routers):
        ...
        for cost in costs:
            if (addr == tuple(cost[0]) and tuple(cost[1]) in online_routers) or (
                    addr == tuple(cost[1]) and tuple(cost[0]) in online_routers):
    ...
    def update_next_hop(self, local_addr):
        ...
        for router in self.routers_list:
            ...
            flag[temp] = 1
            for router in self.routers_list:
                if self.cost[tuple(router)] > self.cost[temp] + self.line_state[temp][tuple(router)] and flag[
                    tuple(router)] == 0:
